import { collection, addDoc, getDocs, setDoc, doc, query, limit, where, getDoc } from 'firebase/firestore';
...
const getWorkList = async (userid: string, maxWorkCount?: number) => {
  const workDatabase = await (
    maxWorkCount ? getDocs(query(workCollection, where("userid", '==', userid), limit(maxWorkCount))) : getDocs(query(workCollection, where("userid", '==', userid)))
  );
  return snapshotToList(workDatabase, userid);
};

const updateWork = (id: string, workDetail: WorkDetail) => setDoc(doc(workCollection, id), workDetail);

const addWork = (workDetail: WorkDetail) => addDoc(workCollection, workDetail);

// eslint-disable-next-line no-return-await
const createUser = async (userid: string, name: string) => await setDoc(doc(userCollection, userid), {
    userid,
    name,
  });
