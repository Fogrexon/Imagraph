...
const Notification = ({
  ...
  return (
    <div className={`${className} z-50 w-full fixed transition-transform`}>
      <div
        className={`mx-auto my-5 w-90 z-50 max-w-lg w-full ${style[0]} border-t-4 ${style[1]} rounded-b ${style[2]} px-4 py-3 shadow-md`}
        role="alert"
      >
        <div className="flex">
...
export const NotificationProvider = ({ children }: NotificationPrividerProps) => {
  ...
  return (
    <NotificationContext.Provider value={{ dispatchNotification }}>
      <Notification className={notificationClass} type={type}>
        {message}
      </Notification>
      {children}
