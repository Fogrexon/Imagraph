...
export const Viewer = ({glsl, updateErrors, errors}: { glsl: string, updateErrors: (errors: Ace.Annotation[]) => void, errors: Ace.Annotation[] }) => {
  ...
  const createFilter = () => {
    ...
    const initialize = (img: HTMLImageElement) => () => {
      ...
      try {
        ...
        const errorMes: Ace.Annotation[] = `${e}`.split('ERROR: ').slice(1).map(mes => {
  ...
  return (
    <div className="relative flex justify-center w-full">
      ...
      <div className={`absolute top-0 left-0 w-full h-full flex flex-col justify-center items-center transition-opacity duration-500 bg-gray-700 bg-opacity-70 ${errors.length === 0 ? 'opacity-0' : 'opacity-100'}`}>
        <div className="w-11/12">
          {errors.map(error => <Alert type="alert" key={error.text}>{`Line: ${error.row} : ${error.text}`}</Alert>)}
        </div>
      </div>
    </div>
  )
